...
class AppStore extends Component {
  ...
  filterList = () => {
    ...
    const newList = appsList.filter(
      item =>
        item.category === activeTabId &&
        item.appName.toLowerCase().includes(inputText.toLowerCase()),
    )
    return newList
  }
  ...
  render() {
    ...
    return (
      <div className="page-bg">
        <h1 className="page-heading">App Store</h1>
        ...
        <ul className="tabs-container">
          {tabsList.map(item => (
            <TabItem
              ...
              func={this.stateChange}
            />
          ))}
        </ul>
        <ul className="apps-container">
          {filteredList.map(item => (
            <AppItem key={item.appId} appItem={item} />
          ))}
        </ul>
      </div>
    )
